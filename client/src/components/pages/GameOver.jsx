...
const GameOver = () => {
	...
	useEffect(() => {
		setTimeout(() => {
			setMessageDiamonds(`U have won ${diamondPoints} Diamond Points`);
		}, 5000);
	...
	return (
		<div className='gameOver--section'>
			<div>
				...
				{messagePie ? (
					<img className='popUp2' src={pie3} alt='Game_Over' />
				) : (
					<img src={x} alt='Game_Over' />
				)}
			</div>

			{<p className='cat'>{messageDiamonds}</p>}

			{pie === true && <p className='cat'>{messagePie}</p>}
		</div>
	);
};
